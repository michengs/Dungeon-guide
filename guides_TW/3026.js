...
module.exports = (dispatch, handlers, guide, lang) => {
			...
			dispatch.setTimeout(() => {
				if (debuff != null) {
					handlers.text({
						...
						message_TW: (`冰内 (${qbacting_messages[qbacting].message_TW}) | ${mech_messages[(qbacting + debuff + 1) % 2].message_TW}`)
					});
					...
					handlers.text({
						...
						message_TW: (`冰内 (${qbacting_messages[qbacting].message_TW})`)
					});
				}
			}, 500);
			...
			dispatch.setTimeout(() => {
				if (debuff != null) {
					handlers.text({
						...
						message_TW: (`火内 (${qbacting_messages[qbacting].message_TW}) | ${mech_messages[(qbacting + debuff) % 2].message_TW}`)
					});
					...
					handlers.text({
						...
						message_TW: (`火内 (${qbacting_messages[qbacting].message_TW})`)
					});
				}
			}, 500);
	...
	const skills = {
		...
		// AOE огонь (большой)
		"105-0": [
			{ type: "text", sub_type: "message", message: "Fire Bombs", message_RU: "Огненные бомбы", message_TW: "炸弹" },
		...
		// AOE лед (малый)
		"154-0": [
			{ type: "text", sub_type: "message", message: "Ice Storm", message_RU: "Ледяной шторм", message_TW: "冰风暴" },
